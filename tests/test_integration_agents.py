...
@requires_claude_cli
@pytest.mark.asyncio
async def test_clean_transaction_not_flagged(transactions, rag: PolicyRAG) -> None:
    txn = next(t for t in transactions if t.transaction_id == "TXN-0001")
    verdict, _ = await _audit_one(txn, transactions, rag)
    assert verdict.flagged is False


@requires_claude_cli
@pytest.mark.asyncio
async def test_duplicate_payment_flagged_and_confirmed(transactions, rag: PolicyRAG) -> None:
...
@requires_claude_cli
@pytest.mark.asyncio
async def test_false_positive_redteam_cleared(rag: PolicyRAG) -> None:
    result, _ = await _run_false_positive_redteam(rag)
